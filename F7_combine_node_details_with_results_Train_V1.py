...
def make_flat_csv(node_csv, result_csv, output_csv):
    ...
    preferred = [c for c in [
        ...
        "s11", "s22", "s33", "s12", "s23", "s13","von_mises"
    ] if c in df.columns]
...
input_root = Path(r"D:\Agentic_AI\Plate_with_a_hole\Step5_1991_test_cases_plate_model\Train_data")
output_root = input_root / "flat_csvs_1"
output_root.mkdir(exist_ok=True)

result_files = sorted(input_root.glob("*results_train_1.csv"))
node_files = sorted(input_root.glob("*nodal_details.csv"))

for result_csv, node_csv in zip(result_files, node_files):
    out_name = result_csv.name.replace("results_train_1.csv", "flat.csv")
